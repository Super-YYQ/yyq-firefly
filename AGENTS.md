...
## Generated Synced Content

`src/content/posts` files tracked by `.private-content-sync.json` are generated by the `Sync private knowledge base` workflow from `Super-YYQ/yyq-firefly-private`.

- Do not edit these files directly in this repository.
- Content changes belong in the private knowledge base; the next sync overwrites local edits here.
- Other hand-authored posts and all non-post source remain editable normally.
